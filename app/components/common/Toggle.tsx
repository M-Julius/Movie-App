...
function SwitchAccessibilityLabel(props: ToggleInputProps & { role: "on" | "off" }) {
  ...
  const color = (function () {
    if (disabled) return colors.palette.blackTransparent
    if (status === "error") return colors.error
    if (!on) return innerStyle?.backgroundColor || colors.palette.lightGrey
    return detailStyle?.backgroundColor || colors.palette.lighterGrey
  })()
